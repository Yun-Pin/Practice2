...
```{r ans11}
library(dplyr)
classSurvey$課外活動=="團隊活動" -> A
classSurvey$課外活動=="團隊活動, 學會" -> B
classSurvey$課外活動=="團隊活動, 打工" -> C
classSurvey$課外活動=="打工, 團隊活動" -> D
mutate(
  classSurvey,
  性別=as.factor(性別),
  入學年=str_sub(學號,2,4),
  團隊活動=(課外活動 %in%  A|B|C|D|A),
  ) -> classSurvey
...
classSurvey %>%
  group_by(性別) %>%
  summarise(團隊活動=n()) 
sum(as.numeric(classSurvey$團隊活動))
41/60
